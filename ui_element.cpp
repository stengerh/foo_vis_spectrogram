...
// {22C36D2C-31D7-4bd0-B8C8-79142E11D353}
static const GUID guid_element_spectrum = { 0x22c36d2c, 0x31d7, 0x4bd0, { 0xb8, 0xc8, 0x79, 0x14, 0x2e, 0x11, 0xd3, 0x53 } };

class spectrum_instance : public ui_element_instance
{
	CVisualisationPanelHost m_wndView;
public:
	spectrum_instance(HWND p_parent, ui_element_instance_callback_ptr p_callback) {
		m_wndView.set_callback(p_callback);
		HWND hWnd = m_wndView.Create(p_parent, 0, _T("Spectrum"));
	}
	...
	virtual bool edit_mode_context_menu_get_description(unsigned p_id, unsigned p_id_base, pfc::string_base & p_out)
	{
		...
		{
		case ID_SETTINGS:
			p_out = "Shows settings for spectrum visualisation.";
			return true;
...
class spectrum : public ui_element {
public:
	virtual GUID get_guid() {
		return guid_element_spectrum;
	}
	...
	virtual ui_element_instance_ptr instantiate(HWND p_parent, ui_element_config::ptr cfg, ui_element_instance_callback_ptr p_callback) {
		service_ptr_t<spectrum_instance> instance;
		instance = new service_impl_t<spectrum_instance>(p_parent, p_callback);
...
static service_factory_single_t<spectrum> foo_spectrum;
