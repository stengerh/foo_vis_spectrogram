...
pfc::instance_tracker_server_t<CSpectrumView> CSpectrumView::g_instances;
...
LRESULT CSpectrumView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	m_column = 0;
	m_last_time = 0;
...
void CSpectrumView::CheckStreamUpdates()
{
...
void CSpectrumView::OnPaint(HDC hDC)
{
	CPaintDC dc(m_hWnd);
...
void CSpectrumView::OnSize(UINT nType, CSize size)
{
...
void CSpectrumView::OnEnterSizeMove()
{
	m_size_changing = true;
}

void CSpectrumView::OnExitSizeMove()
{
	m_size_changing = false;
	CRect rcClient;
	GetClientRect(&rcClient);
...
void CSpectrumView::on_visualisation_timer()
{
...
bool CSpectrumView::EnableUpdates()
{
...
bool CSpectrumView::DisableUpdates()
{
...
void CSpectrumView::on_playback_starting(play_control::t_track_command p_command, bool p_paused)
{
	m_last_time = 0.0;
	EnableUpdates();
...
void CSpectrumView::on_playback_stop(play_control::t_stop_reason p_reason)
{
	DisableUpdates();
	ClearDisplayCache();
	RedrawWindow(NULL, NULL, RDW_INVALIDATE);
...
void CSpectrumView::on_playback_pause(bool p_state)
{
	if (p_state)
		DisableUpdates();
	else
...
void CSpectrumView::on_playback_seek(double p_time)
{
	m_last_time = 0.0;
}

ATL::CWndClassInfo& CSpectrumView::GetWndClassInfo()
{
...
void CSpectrumView::ResizeDisplayCache(CSize size)
{
...
void CSpectrumView::ClearDisplayCache()
{
	CWindowDC dcWin(m_hWnd);
	CDC dcMem = ::CreateCompatibleDC(dcWin);
...
void CSpectrumView::on_spectrum_colors_changed(const t_spectrum_color_info & p_info)
{
...
void CSpectrumView::set_smooth_scaling(bool p_state)
{
...
void CSpectrumView::UpdateColorMapper()
{
	on_spectrum_colors_changed(cfg_spectrum_color_info);
}

void CSpectrumView::register_play_callback()
{
...
void CSpectrumView::unregister_play_callback()
{
	static_api_ptr_t<play_callback_manager> pcm;
...
{
	t_size count = CSpectrumView::g_instances.get_count();
	for (t_size n = 0; n < count; n++)
		CSpectrumView::g_instances[n]->on_spectrum_colors_changed(p_info);
}
